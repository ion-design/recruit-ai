// ion/Select: Generated with Ion on 2/10/2024, 12:29:10 PM
...
>(({ className, children, iconLeading, ...props }, ref) => (
	<SelectPrimitive.Trigger
		...
		className={twMerge(
			clsx(
				'focus:border-stroke focus:primary-focus disabled:border-stroke-weak group flex h-9 w-full items-center justify-between rounded-radius border border-sub-stroke px-3 py-2 text-sm placeholder:text-soft-foreground disabled:pointer-events-none disabled:bg-weak',
				className
			)
		)}
		{...props}
	...
	(
		...
		<div className={twMerge(clsx('w-full', className))}>
			<SelectBase {...props}>
				...
				{props.hint && (
					<Hint showIcon={showHintIcon} disabled={props.disabled} error={error} className="mt-1">
						{props.hint}
					</Hint>
				)}
