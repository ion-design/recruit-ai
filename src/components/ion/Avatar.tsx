'use client';
// ion/Avatar: Generated with Ion on 2/10/2024, 12:29:12 PM
...
type AvatarStatusType = 'online' | 'offline' | 'inactive' | 'away' | 'check' | 'plus' | 'delete';
...
>(({ className, type, location, size, ...props }, ref) => {
	...
	return (
		<div
			...
			{...props}
		>
			{type === 'check' && <Check className={iconClassNames} />}
			{type === 'plus' && <Plus className={iconClassNames} />}
			{type === 'delete' && <X className={iconClassNames} />}
		</div>
	);
});
...
>(
	(
		{ className, initials, topStatus, bottomStatus, size = 'lg', src, title, subtitle, ...props },
		ref
	) => (
		<div
			className={clsx({
		...
		>
			<AvatarPrimitive.Root
				...
				{...props}
			>
				{topStatus && <AvatarStatus location="top" type={topStatus} size={size} />}
				{bottomStatus && <AvatarStatus location="bottom" type={bottomStatus} size={size} />}

				<AvatarImage src={src} />
				<AvatarFallback initials={initials} size={size} />
			</AvatarPrimitive.Root>
			{(title || subtitle) && <AvatarLabel title={title} subtitle={subtitle} size={size} />}
		</div>
	)
);
